...
InputCue 是一个克制的 Windows 输入状态提示器。在可编辑位置停顿时，它会在光标附近提示中文、输入法英文、美式键盘或大写锁定状态；实际输入时自动隐藏，并综合当前编辑环境与可用位置，尽量减少无关场景的误触发。
...
<p align="center">
  <img src="./assets/readme/indicator-styles.svg" width="1200" alt="默认、描边、阴影、圆点和两个自定义槽位，可展示中文、输入法英文、美式键盘和大写锁定四种状态。">
</p>

- **默认**：新用户的首选槽位，使用白色字形、柔和深色连续圆角底和半透明白色细描边。
- **描边**：黑白硬阴影，识别度最高。
- **阴影**：浅色卡片与柔和阴影，更自然地融入桌面。
- **圆点**：用四种颜色表达状态，存在感最低。

六个槽位都可独立设置位置、偏移、尺寸和切换动画；圆点还可自定义四种状态颜色。「自定义1」和「自定义2」可分别用自己的 PNG 图标替换各状态图标，详见[自定义图标](#自定义图标)。

## 自定义图标

把样式切换为「自定义1」或「自定义2」后，可为中文、输入法英文、美式键盘和大写锁定四个状态分别指定 PNG 图片；两个槽位的图片和阴影设置彼此独立：
...
- **更少打扰**：同一应用内切换输入框的重复提示可选始终显示、延时显示或始终不显示；输入模式改变时仍会提示。
- **输入空闲提示**：默认在活动输入位置保持显示，实际编辑内容时隐藏，停止输入 3 秒后重新显示；仅移动光标时会继续跟随，原有短暂提示模式仍可选。
...
逐程序、逐编辑场景的完整支持、部分支持、不支持与未验证状态，由共享识别模块 TextPicker 维护在[兼容性清单](https://github.com/huangko555/TextPicker/blob/main/docs/compatibility.md)中。
