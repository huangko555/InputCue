...
当前实现策略：先用描边、阴影和圆点三种样式、固定四态视觉、默认 1 秒显示、输入后最短显示 300ms 和单一启用开关跑通端到端流程。三种样式分别保存对齐、尺寸与横纵偏移，切换样式时设置页恢复对应参数；旧配置首次读取时继承原公共位置和偏移。同一应用内的重复提示由三档模式控制（始终显示、延时后显示、始终不显示），编辑活动后的 1.5 秒上下文重播抑制仅作为 UIA 身份抖动的低成本兜底，不增加轮询或 Hook；主题和后续样式等待独立设计后再实现。
